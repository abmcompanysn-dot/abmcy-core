...
/** Logique + rendu de la page Commandes, partagés entre le rendu desktop
 * (tableau, dans app/page.tsx) et le rendu mobile (cartes, dans
 * app/commandes/page.tsx). `variant` choisit uniquement la présentation
 * de la liste — le chargement, le filtrage et la création sont
 * identiques. */
export function OrdersPageContent({
  ...
  // moment, donc filtrer la liste déjà chargée est suffisant plutôt que
  // de repenser la pagination serveur.
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className={mobile ? "space-y-3" : "flex flex-wrap items-end gap-3"}>
          <div className={mobile ? undefined : "min-w-[220px] flex-1"}>
          ...
          <div className={mobile ? "flex gap-3" : "contents"}>
            <div className={mobile ? "flex-1" : undefined}>
              <label className="mb-1 block text-xs font-medium text-slate-500">
                Depuis le
              </label>
              <input
      ...
      {loading && orders === null ? (
        ...
        <>
          ...
          {mobile ? (
            <OrdersCardList orders={filteredOrders ?? []} />
          ) : (
